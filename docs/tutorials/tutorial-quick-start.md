...
This is the quick start for using a Scientific Filesystem. We won't be designing or creating one, but using a pre-generated Docker or Singularity container. The awesome part is that despite two different container technologies, the guts inside, and interacting with them, will be the same! If you *do* want to learn these first steps, see the [preview and install tutorial](/scif/tutorial-preview-install).
...
## Singularity
For this second example, we are using the Singularity scientific filesystem container that is built in the [preview and install](/scif/tutorial-preview-install#install-scif-in-singularity-using-recipe) tutorial. First, you should pull the image, either with the [sregistry client](https://singularityhub.github.io/sregistry-cli) or singularity natively:
